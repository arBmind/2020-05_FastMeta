...
template<class T, T V> constexpr //
    Const<T, V>* const_ = {};
...
constexpr auto bench_d(const void* a, std::index_sequence<Is...>) {
    (void)(sizeof(type<Const<std::size_t, Is>>) + ...);
#ifdef BASELINE
    return true;
#else
    return (stx::same(&type<Const<std::size_t, Is>>, a) || ...);
#endif
}
