...
template<class _Tp, _Tp _v> struct integral_constant {
...
template<class _Tp, _Tp _v> constexpr const _Tp integral_constant<_Tp, _v>::value;
...
template<class T, size_t... Is> constexpr auto bench_impl(std::index_sequence<Is...>) {
    ((void)stx::integral_constant<size_t, Is>{}, ...);
    return true;
}

template<size_t... Is> constexpr auto bench2_impl(std::index_sequence<Is...>) {
    return (bench_impl<stx::integral_constant<size_t, Is>>(std::make_index_sequence<N>()) || ...);
}
